...
# 2,type(name, bases, dict) //传入三个参数时，返回一个新的 type 对象。 这在本质上是 class 语句的一种动态形式。
...
# python中类创建的本质，当使用 class 创建类时，Python 底层其实使用的是type() 函数来创建，因此可以直接使用 type() 函数来实现动态创建类。
# 元类就是类的类，type() 实际上是一个元类，它创建所有的类，如下：
# 任何对象最终的所属类都是type
# num = 1
# print(num.__class__)  # <class 'int'>  // num的类型是整数
# print(num.__class__.__class__)  # <class 'type'>  //整数的类型是type
...
# 分析，当使用 class 创建类时其实是定义了一个特殊的对象（type 类的对象），并将该对象赋值给 Animal 变量，即 class 定义的所有类都是 type 类的实例。
class Animal():
...
print(type(tiger))  # <class '__main__.Animal'>  //tiger是从Animal类实例化
