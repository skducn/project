...
def b_isElement(browser,resourceId):
    # 判断browser元素是否存在,不存在则返回时间戳
    bIsElementStatus = False
    try:
        browser.find_element_by_id(resourceId)
        bIsElementStatus = True
    except :
        bIsElementStatus = False
        # bIsElementStampTime=u"%s" % datetime.datetime.now().strftime("%Y%m%d%H%M%S%f")[:-3]  #20160804143447945
    return bIsElementStatus
def b_isElementPath(browser,element):
    ...
    try:
        ...
        bIsElementStatus = False
        # bIsElementStampTime=u"%s" % datetime.datetime.now().strftime("%Y%m%d%H%M%S%f")[:-3]
    return bIsElementStatus
...
def b_sysMessage(browser,varPhone,varTitle,varContact,varLink):
    # 消息管理, 给手机发送消息 ,制定发送用户输入框如果为空则指系统内所有用户,如需指定多个用户则用逗号分隔,如 13816109050,13611958544,13012342344
    browser.find_element_by_xpath('//div[@class="nav"]/ul/li[4]').click()    # 运营
    sleep(3)
