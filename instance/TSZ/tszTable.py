...
try :
    if  data0 == None :
        print "Error, 第一步请先用手机号 " + str(varPhone) +" 登录！"
        sleep(121212)
    else:
        myuserID = data0[0]
        mytszNumber = data0[1]
        ...
        #     varAuth = "还未认证哦!"
        #     # 数据库修改为认证
        #     curWeb.execute('update t_redgroup_baseinfo set isAuth=0,authType=0 where userId="%s" order by id desc ' % (myuserID))
        #     connWeb.commit()
        #     varAuth = "操作数据库个人认证通过"
        # #
...
class TszTable(unittest.TestCase):
    ...
    def t_userLogin(self,varDatabaseName,varTableName,varCur):
        ...
        print ">>>>>>> [" + str(varDatabaseName) + " , phone = " + str(varPhone) + ", tszNumber = " + str(mytszNumber) + " , userID = " + str(myuserID) + " , groupID = " + str(mygroupID) + " , " + str(VARPERIODTIME) + "]"
