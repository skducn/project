...
varEndX = 1440 # 手机屏幕分辨率宽度和高度
varEndY = 2392
varEndXY = str(varEndX) + str(varEndY)
ExcelFile = "/Users/linghuchong/Downloads/51/ForWin/Python/03_Project/TSZ/excel/tsz_v2_5.xls"
ReportHtml = "/Users/linghuchong/Downloads/51/ForWin/Python/03_Project/TSZ/report/testreport_v2_5.html"
...
newbk=copy(bk)
sheetMain= bk.sheet_by_name("configure")
...
class tsz(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        desired_caps['platformVersion'] = sheetMain.cell_value(1,7)  #'7.0'
        desired_caps['deviceName'] =  sheetMain.cell_value(1,8) #'NP5B270003'  #'6c9cb8bf'
        desired_caps['appPackage'] = sheetMain.cell_value(1,9) #'com.mowin.tsz'
        desired_caps['appActivity'] = sheetMain.cell_value(1,10) #'com.mowin.tsz.SplashActivity'
    ...
    def test_Main(self):
        for i in range(1,sheetMain.nrows):         #遍历Main执行函数模块
    ...
    def TestcaseModule(self):
         #遍历TestCase及调用函数模块
         case1=caseN=0
         for j in range(1,sheetTestCase.nrows):               # 定位测试用例位置及数量
              case1=case1+1
              if sheetTestCase.cell_value(j,1) == self.Maincol1 and sheetTestCase.cell_value(j,2) == self.Maincol2:
                  for k in range(case1+1,100): # 假设有100个Case
                      if k + 1 > sheetTestCase.nrows:  # 最后一行
                           ...
                           break
                  break
         for l in range(case1,caseN+case1):         #遍历 Testcase1~TestCaseN
              str_list=[]
              ...
              try :
                  if sheetTestCase.cell_value(l,1)=="skip" or sheetTestCase.cell_value(l,2)=="skip":
...
# self.driver.switch_to_default_content()
    ...
    def homeJumpLogin(self):
        ...
        self.driver.find_element_by_id("com.mowin.tsz:id/next").click() # 下一步
        if self.isElement("com.mowin.tsz:id/phone_number")==True:
           self.driver.swipe(100, 100, 100, 100, 500)  # 无密码快捷登录 页面,点击 左上角的关闭,应返回首页
