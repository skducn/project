...
def Idefault(varnum,varuserId,varInterfaceName,varUrl,testcase,**query):
     ...
     try:
        if varnum=="RtnNullOK" :  #如:{"data":null,"errorstr":"","errorcode":0,"success":true}
            ...
            else:print "[errorrrrrrrrrr,RtnNullOK]," + varInterfaceName + " => " + testcase + " => " + response.content
...
def I24_1bak(varnum,varuserId,param1,param2,param3,testcase):
    r = redis.StrictRedis(host='192.168.2.176', port=6379, db=0,password="dlhy123456")
    varverifyCode = r.hget("app:verify:"+varuserId,"code")
    # 参数: userId = 用户ID, likeName = 搜索名称(可选), industryId = 分类(可选)
    ########################################################################
    varInterfaceName =  "I24_1,更多红包群接口(update)"
    varUrl = "http://192.168.2.176:9999/WebBusi/redGroupInfo/2.2/all_redGroup.do"
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"userId":param1,"likeName":param2,"industryId":param3}
    ########################################################################
    headers = {'cache-control': "no-cache"}
...
def I24_2(varnum,varuserId,param1,param2,param3,param4,param5,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"userId":param1,"likeName":param2,"industryId":param3
        ,"startIndex":param4,"pageSize":param5}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_3(varnum,varuserId,param1,param2,param3,param4,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"userId":param1,"groupId":param2,"labelName":param3,"memberIds":param4}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_4(varnum,varuserId,param1,param2,param3,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"id":param1,"groupId":param2,"labelName":param3}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_6(varnum,varuserId,param1,param2,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"id":param1,"groupId":param2}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_7(varnum,varuserId,param1,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"id":param1}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_8(varnum,varuserId,param1,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"groupId":param1}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_10(varnum,varuserId,param1,param2,param3,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"userId":param1,"groupId":param2,"remarks":param3}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_11(varnum,varuserId,param1,param2,param3,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"userId":param1,"groupId":param2,"isBlack":param3}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_12(varnum,varuserId,param1,param2,param3,param4,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"userId":param1,"groupId":param2,"oldGroupLabel":param3,"groupLabel":param4}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
...
def I24_13(varnum,varuserId,param1,param2,param3,testcase):
    ...
    querystring = {"verifyUserId":varuserId,"verifyCode":varverifyCode,"userId":param1,"groupId":param2,"nickname":param3}
    if param3=="":
...
def I24_14(varnum,varuserId,param1,param2,testcase):
    ...
    try:
        ...
        print Exception,":",data,"\n"
def I24_15(varnum,varuserId,param1,param2,varNum,testcase):
    r = redis.StrictRedis(host='192.168.2.176', port=6379, db=0,password="dlhy123456")
    varverifyCode = r.hget("app:verify:"+varuserId,"code")
    ...
    try:
        if varnum=="RtnOK" :
            if response.json()['success']==True and len(response.json()['data'])==varNum:
                print "[OK,RtnOK]," + varInterfaceName + " => " + testcase + " => " + response.content
            else:
                print "[errorrrrrrrrrr,RtnOK]," + varInterfaceName + " => " + testcase + " => " + response.content
...
def I24_29(varnum, varuserId, param1, param2, testcase):
    ...
    try:
        if varnum == "RtnOK":
            if response.json()['success'] == True and response.json()['data'] <> None:
                print "[OK,RtnOK]," + varInterfaceName + " => " + testcase + " => " + response.content
                return (str(data1['orderVerifCode']),str(data1['orderId']))
            else:
                print "[errorrrrrrrrrr,RtnOK]," + varInterfaceName + " => " + testcase + " => " + response.content
        if varnum == "RtnSysErr":
            if response.json()['errorcode'] == 100001 and response.json()['success'] == False:
        ...
        print Exception, ":", data, "\n"
def I24_30(varnum, varuserId, param1, param2, param3, param4, testcase):
    r = redis.StrictRedis(host='192.168.2.176', port=6379, db=0, password="dlhy123456")
    varverifyCode = r.hget("app:verify:" + varuserId, "code")
    # 参数: userId = 用户ID, amount = 交易金额, orderVerifCode=交易验证码,payOrderId =交易订单号
    ########################################################################
    varInterfaceName = "I24_30,账户微信充值回调接口"
    varUrl = "http://192.168.2.176:9999/payment/order/1.0/accountOrderReturn.do"
    querystring = {"verifyUserId": varuserId, "verifyCode": varverifyCode, "userId": param1,
                       "amount": param2, "orderVerifCode": param3, "payOrderId": param4}
    ########################################################################
    headers = {'cache-control': "no-cache"}
    response = requests.request("GET", varUrl, headers=headers, params=querystring)
    try:
        if varnum == "RtnOK":
            if response.json()['success'] == False and response.json()['data'] == "您取消了支付！" :
                print "[OK,RtnOK]," + varInterfaceName + " => " + testcase + " => " + response.content
            else:
                print "[errorrrrrrrrrr,RtnOK]," + varInterfaceName + " => " + testcase + " => " + response.content
        if varnum == "RtnSysErr":
...
print "15,================================================================================================="
# 15 红包群成员_获取所有群成员接口(完成)
# 业务逻辑: 检查表 t_redgroup_memberinfo ,条件:groupId=213 且 isBlack=0 的所有成员.
# 从表中获取符合条件的个数,譬如 将个数存入data1[0],并与接口Json返回的数量对比.
conn= MySQLdb.connect(host='192.168.2.176', user='ukaapp', passwd='ukaapp', db='ukardweb', port=3306, use_unicode=True) #sit2
curT = conn.cursor()
curT.execute('select count(id) from t_redgroup_memberinfo where groupUserId=10001679 and groupId=213 and isBlack=0')
data1 = curT.fetchone()
curT.close(),conn.close()
...
(varOrderVerifCode,varOrderId) = I24_29("RtnOK", "10001679", "10001679", "90.01", "C29-5,用户ID,充值金额（元）")
...
I24_30("RtnOK", "10001679", "10001679", "90.01", varOrderVerifCode, varOrderId, "C30-1,用户ID,交易金额,交易验证码,交易订单号")
