...
data2 = pd.read_excel(file_dir_name, sheet_name='format')


# 检查重复值
# 检查指定列重复值的数量
print(data2.duplicated(['最新价'], keep=False).sum())
# 检查行记录重复值的数量
print(data2.duplicated(keep=False).sum())
# 删除指定列的重复数据，保留重复行的最后一行
# data2.drop_duplicates(['最新价'],keep='last')
# 直接删除，保留一个副本
# data2 = data2.drop_duplicates(['最新价'], inplace=False)
sys.exit(0)
...
# 将'最新价'的空值替换成0
# data2['最新价'].fillna(0, inplace=True)

# 获取标题
print(list(data2.loc[0:]))  # ['序号', '股票代码', '股票简称', '最新价', '涨跌幅', '换手率', '流入资金', '流出资金', '净额', '成交额']

# 获取记录中包含空值的行数据
print(data2.loc[data2.isnull().values])
...
# 获取记录中包含空值的索引值
print(data2.loc[data2.isnull().values].index)  # Index([11, 13], dtype='int64')
print(list(data2.loc[data2.isnull().values].index))  # [11, 13]
print(list(data2.loc[data2.isnull().values].index)[0])  # 11


# 获取记录中包含空值的序号
print(data2.loc[data2.isnull().values, '序号'])
# 11    12
# 13    14
# Name: 序号, dtype: int64
print(list(data2.loc[data2.isnull().values, '序号']))  # [12, 14]

print(list(data2.loc[data2.isnull().values, '序号'])[1])  # 14

print(len(data2.loc[data2.isnull().values]))  # 2

# 删除包含缺失值的行记录
# data2.dropna(inplace=True)
# print(data2.head(20))
data2.sort_values('流入资金', inplace=True, ascending=False)
print(data2.head(20))

# print(data2.isnull().sum())
# # print(data2['最新价'].head(10))

# sheet_name不能修改已经存在的sheet，必须新的sheet
# with pd.ExcelWriter(file_dir_name, engine='openpyxl', mode='a') as writer:
#     data2.to_excel(writer, sheet_name='formatFillna', index=False)
