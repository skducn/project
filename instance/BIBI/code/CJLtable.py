...
class TszTable(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        print u"[有效时间段: " + self.myfrtTime+" ~ " + self.myEndTime +"]"

        # print "t_user => [" + data0[1] +","+ str(data0[0])+","+ str(data0[2])+ u",可提现金:"+str(data0[6])+u",上次提现时间:"+str(data0[7])+u",提现金额:"+str(data0[8])+u",充值金额:"+str(data0[9]) +u",注册时间:"+str(data0[3])+u",登录时间:"+str(data0[4])+u",首次登录时间:"+str(data0[5])
        # 账户余额 = 佣金余额 + 充值金额(大咖)
    ...
    def t_userLogin(self,cur,tables):
        # C1-1, 用户第一次登录app,检查在那些表中生成记录.
        # 遍历所有表
        for AT in tables:            #print AT[0]  # 所有表
            # 获取表中所有字段
            cur.execute("select * from %s" % AT[0])
            col_name_list = [tuple[0] for tuple in cur.description]
            # 判断表中id字段, 如果不存在则跳过此表, 否则遍历此表统计数量,忽略空表.
            if 'id' in col_name_list:
                cur.execute('select count(id) from %s' % (AT[0]))
                t1 = cur.fetchone()
                if t1[0] <> 0:
                    # 列表 phone = {myPhone}的表 ,检查字段 phone
                    if 'phone' in col_name_list:
                        cur.execute('select count(id) from %s where phone="%s" order by id desc ' % (AT[0],myPhone))
                        t2=cur.fetchone()
                        if t2[0]<>0:
                    ...
                    if 'username' in col_name_list:
                        ...
                        cur.execute('select count(id) from %s where username="%s" order by id desc ' % (AT[0],myPhone))
                        t2=cur.fetchone()
                        if t2[0]<>0:
                            print AT[0] + u" => [username(" + str(myPhone) + ") = " + str(t2[0]) + "]"

                    if 'user_id' in col_name_list:
                        cur.execute('select count(id) from %s where user_id="%s" order by id desc ' % (AT[0],self.myuserID))
                        t2=cur.fetchone()
                        if t2[0]<>0:
                            print AT[0] + u" => [user_id(" + str(self.myuserID) + ") = "+  str(t2[0]) + "]"

                    if 'id' in col_name_list:
                        cur.execute('select count(id) from %s where id="%s" order by id desc ' % (AT[0],self.myuserID))
                        t2=cur.fetchone()
                        if t2[0]<>0:
                            print AT[0] + u" => [id(" + str(self.myuserID) + ") = "+  str(t2[0]) + "]"

                    if 'userid' in col_name_list:
                        cur.execute('select count(id) from %s where userid="%s" order by id desc ' % (AT[0],self.myuserID))
                        t2=cur.fetchone()
                        if t2[0]<>0:
                    ...
                    if 'userId1' in col_name_list:
                        cur.execute('select count(id) from %s where userId1="%s" order by id desc ' % (AT[0],self.myuserID))
                        t2=cur.fetchone()
                        if t2[0]<>0:
                            print AT[0] + " => [userId1(" + str(self.myuserID) + ") = "+  str(t2[0]) + "]"
                    ...
                    for m in range(len(col_name_list)):
                         ...
                         if m1:
                             ...
                             if col_name_list[m]  in col_name_list:
                                 if 'user_id' in col_name_list:
                                    cur.execute('select count(id) from %s where user_id="%s" order by id desc ' % (AT[0],self.myuserID))
                                    t2=cur.fetchone()
                                    if t2[0]<>0:
                                         ...
                                         cur.execute('select %s from %s where %s BETWEEN "%s" And "%s" and user_id=%s order by "%s" desc' % (col_name_list[m],AT[0],col_name_list[m],self.myfrtTime,self.myEndTime,self.myuserID,col_name_list[m]))
                                         numrows = int(cur.rowcount)    # 循环numrows次，每次取出一行数据
                                         x=""
                                         num=0
                                 ...
                                 if 'userId' in col_name_list:
                                    ...
                                    if t2[0]<>0:
                                         ...
                                         # 符合有效期时间内 的 {myuserID}记录 ,如输出:  t_redgroup_baseinfo => [createTime = (2016-06-06 14:11:53) ]
                                         cur.execute('select %s from %s where %s BETWEEN "%s" And "%s" and userId=%s order by "%s" desc' % (col_name_list[m],AT[0],col_name_list[m],self.myfrtTime,self.myEndTime,self.myuserID,col_name_list[m]))
                                         numrows = int(cur.rowcount)    # 循环numrows次，每次取出一行数据
                                         x=""
                                         num=0
                                         for i in range(numrows):
                                 ...
                                 if 'userId1' in col_name_list:
                                    ...
                                    if t2[0]<>0:
                                         # print AT[0] + u" => [userId,共有 = " +  str(t2[0]) + u"条]" # {myuserID}记录共有N条,无条件限制.
                                         # 符合有效期时间内 的 {myuserID}记录 ,如输出:  t_redgroup_baseinfo => [createTime = (2016-06-06 14:11:53) ]
                                         cur.execute('select %s from %s where %s BETWEEN "%s" And "%s" and userId1=%s order by "%s" desc' % (col_name_list[m],AT[0],col_name_list[m],self.myfrtTime,self.myEndTime,self.myuserID,col_name_list[m]))
                                         numrows = int(cur.rowcount)    # 循环numrows次，每次取出一行数据
                                         x=""
                                         num=0
                                         for i in range(numrows):
                                            ...
                                            num=num+1
                                         if x<>"":
                                             print AT[0] + " => [userId1("+ str(self.myuserID) +") = " + str(num)  + " , " +  col_name_list[m]  +  x + "]"
                                 ...
                                 if 'id' in col_name_list:
                                     ...
                                     if t2[0]<>0:
                                         ...
                                         # 符合有效期时间内 的 {myuserID}记录 ,如输出:  t_redgroup_baseinfo => [createTime = (2016-06-06 14:11:53) ]
                                         cur.execute('select %s from %s where %s BETWEEN "%s" And "%s" and id=%s order by "%s" desc' % (col_name_list[m],AT[0],col_name_list[m],self.myfrtTime,self.myEndTime,self.myuserID,col_name_list[m]))
                                 ...
                                 if 'phone' in col_name_list:
                                    ...
                                    if t2[0]<>0:
                                         ...
                                         # 符合有效期时间内 的 {myuserID}记录 ,如输出:  t_redgroup_baseinfo => [createTime = (2016-06-06 14:11:53) ]
                                         cur.execute('select %s from %s where %s BETWEEN "%s" And "%s" and phone=%s order by "%s" desc' % (col_name_list[m],AT[0],col_name_list[m],self.myfrtTime,self.myEndTime,myPhone,col_name_list[m]))
                                         numrows = int(cur.rowcount)
                                         x=""
                                         num=0
                                         for i in range(numrows):
                                 ...
                                 if 'username' in col_name_list:
                                    ...
                                    if t2[0]<>0:
                                         ...
                                         cur.execute('select %s from %s where %s BETWEEN "%s" And "%s" and username=%s order by "%s" desc' % (col_name_list[m],AT[0],col_name_list[m],self.myfrtTime,self.myEndTime,myPhone,col_name_list[m]))
                                         numrows = int(cur.rowcount)
                                         x=""
                                         num=0
                                         for i in range(numrows):
