...
# 参数化
varExcel = "/Users/linghuchong/Downloads/51/Project/Bibi/excel/Bibi1_0.xls"
...
class cjl(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        desired_caps['appPackage'] = 'com.kuping.android.boluome.life.debug'
        ...
        sleep(5)
        # self.driver.find_element_by_xpath("//android.widget.TabHost/android.widget.LinearLayout/android.widget.TabWidget/android.widget.RelativeLayout[contains(@index,1)]").click()
        # self.driver.find_element_by_xpath("//android.widget.TabHost/android.widget.LinearLayout/android.widget.TabWidget/child::android.widget.RelativeLayout[1]").click()
        self.driver.find_element_by_id("android:id/tabs").find_element_by_xpath("//android.widget.RelativeLayout[contains(@index,1)]").click()
        ...
        def permission_Tab_brands3():
            WebDriverWait(
                ...
                method=EC.visibility_of_element_located(
                    (By.XPATH, ("//android.widget.TabWidget/android.widget.RelativeLayout[3]"))),
   ...
   # #判断app元素是否存在,不存在则返回时间戳
    ...
    def checkSceneType(self,sceneName,exlSheet,sheetNum,id0,id1,id2,id3,ignoreBeforeNums):
        ...
        if len(l_one) != len(l_exlone):
            print "[Errorrrrr, " + sceneName + " , 预期(" + str(len(l_exlone)) + ")与实测(" + str(len(l_one)) +")数量不一致!]"
        else:
            for j in range(self.exlColnums(exlSheet, 1)):
                newWs = newbk.get_sheet(sheetNum)
                if l_one[j] in l_exlone[j] :
        ...
        if int(l_onenum%dslen) == 0: xy = int(l_onenum/dslen)
        else: xy = int(l_onenum/dslen)+1
