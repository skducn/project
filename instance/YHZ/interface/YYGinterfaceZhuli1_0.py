...
# 职称列表
resp = response(u"[职称列表]",'/v1/system/getTitleList','{}','y')

# # 找回密码或者更新密码
# # 忘记密码,type = 0
# myMd5.update("jinhao123")
# myMd5_Digest = myMd5.hexdigest()
# resp = response(u'找回密码','/v1/user/findPassWord',"{\"phoneNumber\":\"" + varPhone + "\",\"passWord\":\"" + myMd5_Digest + "\",\"verificationCode\":\"" + str(redisCode) + "\",\"identityType\":\"3\",\"type\":\"0\"}",'y')


# 登录
resp = response(u"[登录]",'/v1/user/login',"{\"phoneNumber\":\"" + varPhoneZhuli + "\",\"passWord\":\"" + myMd5_Digest + "\",\"identityType\":\"3\"}",'y')
...
resp = response(u"[助理首页]",'/v1/user/assistantHome',"{\"userId\":\"" + varUserId + "\",\"userSessionId\":\"" + varUserSessionId + "\"}",'y')
...
# 上传图片，fileTye(1=头像，2=认证，4=病例),   # 注：上传图片与修改头像需一起执行。
...
# 修改头像 , ImageUrl从上传图片返回中获取，# varImageUrl = resp.split("imageUrl\":\"")[1].split("\"")[0]
resp = response(u"[修改头像]",'/v1/doctorCenters/DoctorImageUrl',"{\"userId\":\"" + varUserId + "\",\"imageUrl\":\"" + resp.split("imageUrl\":\"")[1].split("\"")[0] + "\",\"userSessionId\":\"" + varUserSessionId + "\"}",'y')
