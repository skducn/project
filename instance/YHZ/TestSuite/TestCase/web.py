...
# Version    : 1.0.0
# Date       : 2017-7-13
# Description: 云会诊 综合管理平台，10.111.3.5:88/yhzv2/web/app_test.php/curuser/security/login
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
varOfficeNameRandom4 = u"科室" + "".join(Level_PO.myfunc(3))
varSubOfficeNameRandom4 = u"子科室" + "".join(Level_PO.myfunc(4))
...
class Main(unittest.TestCase):
    ...
    def login(self, varUser, varPass):
        ...
        # 登录
        Level_PO.clickXPATH("//button[@class='btn btn-primary btn-block']", 4)
        print u"[Done], " + self.exlTestCase + u"\n"
        return u""

    def institutionCreate(self, varInstituationName, varCode, varType, varRegion, varAddress, varAccount, varName, varPhoneNumber, varOffice, varConsultation, varTeach, varMeeting, var2, var5, var10, var25):

        # institutionCreate(u' 机构1', u'代码1', u'卫计局', u'上海', u'浦东东方路100号', u'abc123', u'金浩', u'13816109050', u'内科', u'开启',
                          # u'开启', u'关闭', u'1', u'2', u'3', u'4')
        ''' 新建机构 '''
        print self.exlTestCase + u" >>>>>>>>>>>>>>>>>>>>>>>>>>"
        ...
        # 点击新建
        Level_PO.clickLINKTEXT(u'新建', 4)

        try:
            # 机构管理 - 添加机构 - 基本信息的填写
            Level_PO.inputID('orgbundle_new_ttorg_name', varInstituationName)
            Level_PO.inputID('orgbundle_new_ttorg_code', varCode)
            Level_PO.selectXPATHtext("//select[@id='orgbundle_new_ttorg_orgType']", varType)
            Level_PO.inputID('orgbundle_new_ttorg_region', varRegion)
            Level_PO.inputID('orgbundle_new_ttorg_address', varAddress)

            # 账号信息
            Level_PO.inputID('orgbundle_new_ttorg_doctors_0_userName', varAccount)
            Level_PO.inputID('orgbundle_new_ttorg_doctors_0_name', varName)
            Level_PO.inputID('orgbundle_new_ttorg_doctors_0_phoneNumber', varPhoneNumber)
            Level_PO.inputID('orgbundle_new_ttorg_doctors_0_office', varOffice)

            # 服务信息(视讯服务）
            Level_PO.selectXPATHtext("//select[@id='orgbundle_new_ttorg_serviceStatusConsultation']", varConsultation)
            Level_PO.selectXPATHtext("//select[@id='orgbundle_new_ttorg_serviceStatusConsultation3']", varTeach)
            Level_PO.selectXPATHtext("//select[@id='orgbundle_new_ttorg_serviceStatusConsultation2']", varMeeting)

            # 服务信息(视讯资源）
            Level_PO.inputID('orgbundle_new_ttorg_totalSourceCountA', var2)
            Level_PO.inputID('orgbundle_new_ttorg_totalSourceCountB', var5)
            Level_PO.inputID('orgbundle_new_ttorg_totalSourceCountC', var10)
            Level_PO.inputID('orgbundle_new_ttorg_totalSourceCountD', var25)

            # 提交
            Level_PO.clickXPATH("//button/span", 2)

            if u'创建下属机构成功' in Level_PO.printXPATH("//div[@class='alert alert-dismissible alert-info no-margin']") :
                print "OK, 提示'创建下属机构成功'"
            else:
                print "errorrrrrrrrrr, def institutionCreate, 未提示'创建下属机构成功'"
        except:
           print "errorrrrrrrrrr, def institutionCreate, 创建机构填写信息有误！！！"

        print u"[Done], " + self.exlTestCase + u"\n"
        return u""

    def officeCreate(self, varInstitutionName, varOfficeName, varSubOfficeName):
        # self.officeCreate(u'jg13', u'唧唧',u'咋咋呼呼')
        self.varInstitutionName = varInstitutionName
        self.varOfficeName = varOfficeName
        self.varSubOfficeName = varSubOfficeName
        ''' 选择机构 ，新建一级科室'''
        print self.exlTestCase + u" >>>>>>>>>>>>>>>>>>>>>>>>>>"

        # 点击科室管理
        Level_PO.clickLINKTEXT(u'科室管理', 4)

        # 选择XXX机构
        Level_PO.selectNAMEtext(u'orgId', varInstitutionName)

        # 点击查询
        Level_PO.clickXPATH("//button[@class='btn btn-info']", 2)

        # 点击新建一级科室
        Level_PO.clickXPATH("//a[@class='btn btn-primary']", 2)

        try:
            Level_PO.inputID("officebundle_tmoffice_officeName", varOfficeName)  # 新建科室 - 科室名称
            Level_PO.clickXPATH("//form[@name='officebundle_tmoffice']/button", 3)  # 提交
        except:
            print "errorrrrrrrrrr, def officeCreate, 新建一级科室有误！！！"

        # 获取机构org_id
        cur.execute('select org_id from tt_org where name="%s" order by org_id desc limit 1' % (varInstitutionName))
        t1 = cur.fetchone()
        # print str(t1[0])
        # 获取一级科室office_id
        cur.execute('select office_id from tm_office where org_id=%s and office_name="%s" order by org_id desc limit 1' % (t1[0], varOfficeName))
        t2 = cur.fetchone()
        print t2[0]
        ...
        # 点击新建子科室
        Level_PO.clickXPATH("//a[@href='/yhzv2/web/app_test.php/office/office_tmoffice/new/" + str(t1[0]) + "/" + str(t2[0]) + "']", 2)
        try:
            Level_PO.inputID("officebundle_tmoffice_officeName", varSubOfficeName)  # 科室名称
            Level_PO.clickXPATH("//form[@name='officebundle_tmoffice']/button", 3)  # 提交
        except:
            print "errorrrrrrrrrr, def officeCreate, 新建子科室有误！！！"

        # 获取子科室office_id
        conn.commit()
        cur.execute('select office_id from tm_office where org_id=%s and office_name="%s" order by org_id desc limit 1' % (t1[0], varSubOfficeName))
        t3 = cur.fetchone()
        # print t3[0]
        print u"[Done], " + self.exlTestCase + u"\n"
        return u""

    def officeEdit(self, varInstitutionName, varOfficeName, varSubOfficeName):

        ''' 编辑子科室 '''

        print self.exlTestCase + u" >>>>>>>>>>>>>>>>>>>>>>>>>>"

        # Level_PO.clickXPATH("//a[@href='/yhzv2/web/app_test.php/office/office_tmoffice/" + str(t2[0]) + "/edit']", 2)  # 编辑
        # varRandom4 = "".join(Level_PO.myfunc(4))
        # Level_PO.inputID("officebundle_tmoffice_officeName", varSubOfficeName + varRandom4)
        # Level_PO.clickXPATH("//form[@name='officebundle_tmoffice']/button", 2)
        print u"[Done], " + self.exlTestCase + u"\n"
        return u""
    ...
    def officeDel(self):
        ...
        # 获取机构org_id
        cur.execute('select org_id from tt_org where name="%s" order by org_id desc limit 1' % (self.varInstitutionName))
        t1 = cur.fetchone()
        # 获取一级科室office_id
        cur.execute('select office_id from tm_office where org_id=%s and office_name="%s" order by org_id desc limit 1' % (t1[0], self.varOfficeName))
        t2 = cur.fetchone()
        # 获取子科室office_id
        cur.execute('select office_id from tm_office where org_id=%s and office_name="%s" order by org_id desc limit 1' % (t1[0], self.varSubOfficeName))
        t3 = cur.fetchone()

        ''' 删除子科室 '''
        Level_PO.clickXPATH("//a[@href='/yhzv2/web/app_test.php/office/office_tmoffice/" + str(t3[0]) + "/delete']", 2)  # 删除子科室
        Level_PO.clickID(u'delete_form_btn_' + str(t3[0]), 2)

        ''' 选择机构 '''
        Level_PO.clickLINKTEXT(u'科室管理', 4)
        Level_PO.selectNAMEtext(u'orgId', self.varInstitutionName)  # 选择机构
        Level_PO.clickXPATH("//button[@class='btn btn-info']", 2)  # 查询

        ''' 删除一级科室 '''
        Level_PO.clickXPATH("//a[@href='/yhzv2/web/app_test.php/office/office_tmoffice/" + str(t2[0]) + "/delete']", 2)  # 删除一级科室
        Level_PO.clickID(u'delete_form_btn_' + str(t2[0]), 2)
