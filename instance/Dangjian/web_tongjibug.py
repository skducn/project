...
# Version    : 1.0.0
# Date       : 2017-8-16
# Description: redmine bug统计（ISO、android、php、server）
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>


from Dangjian.Config.config import *
from Dangjian.PageObject.LoginPO import *
from Dangjian.PageObject.HomePO import *
from Dangjian.PageObject.OperationPO import *

# TaskManage_PO = TaskManagePO(driver, sheetParam.cell_value(1, 1), sheetParam.cell_value(2, 1))
...
class Web(unittest.TestCase):
    ...
    def setUpClass(self):
        pass
        # print u"C1，党建项目管理后台,一级账户创建项目、子项目、资源" http://10.111.3.6:8088/projects/dangjian_v1_5/issues

# def test_L1createProject(self):
    Login_PO = LoginPO(driver, sheetParam.cell_value(1, 1), sheetParam.cell_value(2, 1))
    Home_PO = HomePO(driver, sheetParam.cell_value(1, 1), sheetParam.cell_value(2, 1))
    Operation_PO = OperationPO(driver, sheetParam.cell_value(1, 1), sheetParam.cell_value(2, 1))
    ...
    Login_PO.open(3)
    Login_PO.input_username(u"_username", u"admin")
    Login_PO.input_password(u"_password", u"123456")
    Login_PO.click_submit(3)
    print u"C1，电科党员学习教育平台 - 登录成功"

    #     print u"#1，完成 - 账号'" + sheetParam.cell_value(3, 1) + u"'已登录。"    sleep(1212)
    # Login_PO.selectXPATHtext()
    #
