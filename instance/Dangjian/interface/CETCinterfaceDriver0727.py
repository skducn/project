...
for k, v in d_exlSheet.items():  # 获取第一个工作表名
    if format(v) == "0":exlSheet0 = format(k)


def Icase(exlNo, *params):
    try:
        sheetName = bk.sheet_by_name(exlNo.split("_", 2)[0])  # 定位工作表名
        sheet0 = bk.sheet_by_name(exlSheet0)  # 定位第一个工作表名
    except:
        print "errorrrrrrrrrr, " + exlNo + "的格式有误，请检查工作表名！！！"
    ...
    if floatExlSerialNum in l_floatExlSerialNum:
        for j in range(0, sheetName.nrows):
            ...
            if sheetName.cell_value(j, 0) == floatExlSerialNum :
                ...
                exlRow = d_SerialRow.get(floatExlSerialNum, 'not found')  # 序号所对应的默认序号，输出默认序号
                # print exlRow
                break
        ...
        if len(l_fields) == len(params) and len(l_fields) > 1:
            for i in range(varFields):
                l_fields.append(sheetName.cell_value(exlRow - 1 + i, 5))
        ...
        if varThirdway == 1 and exlIsCancel != 1:
            ...
            try:
                ...
                print "errorrrrrrrrrr, 表格属性名称数量与测试用例提供的数量不一致！！！"
            ...
            if u"操作成功" in str(resp):
                ...
                varResult= 0
                print u"[结果] => [OK] " + exlInterfaceName
                if sheet0.cell_value(3, 0) == "Y":
                    print u"[地址] => " + varURL
                    print u"[请求] => " + str(values)
                    print u"[返回] => " + str(resp)
            else:
                newWs.write(exlRow - 1, 1, "Error", styleRed)
                varResult = 1
                print u"[结果] => [Errorrrrrrrrrr] " + exlInterfaceName
                print u"[地址] => " + varURL
                print u"[请求] => " + str(values)
                print u"[返回] => " + str(resp)
            ...
            if sheet0.cell_value(2, 0) == "Y":
                if varResult == 0:
                    if exlSevendays == "":
                        exlSevendays = str(datetime.datetime.now().strftime('%m%d'))
                    else:
                        if exlSevendays.count("/") == 6:
                            if str(datetime.datetime.now().strftime('%m%d')) not in exlSevendays:
                                ...
                                exlSevendays = str(datetime.datetime.now().strftime('%m%d'))  + exlSevendaysNew
                        else:
                            if str(datetime.datetime.now().strftime('%m%d')) not in exlSevendays:
                                exlSevendays = str(datetime.datetime.now().strftime('%m%d')) + " , " + exlSevendays
        ...
        print "[errorrrrrrrrr , 表格中未找到序号" + str(exlSerialNum) + " ！！！]"
