...
class LoginPO(BasePage):
    ...
    # 定位器，通过元素属性定位元素对象
    # 用户名
    def input_username(self, varByID, username):
        self.find_element(*(By.ID, varByID)).clear()
        self.find_element(*(By.ID, varByID)).send_keys(username)

    # 密码
    def input_password(self, varByID, password):
        self.find_element(*(By.ID, varByID)).clear()
        self.find_element(*(By.ID, varByID)).send_keys(password)
