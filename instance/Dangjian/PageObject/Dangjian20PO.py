...
class Dangjian20PO(object):
    ...
    def flowSetup(self, varExecute, *varObjects):
        ...
        self.Level_PO.clickLINKTEXT(u'流程设置', 2)

        if varExecute == u"del" and varObjects[0] == u"all":
            #  岗位人数
            varPost = self.Level_PO.get_attFromAtts(u"//a[@class='btn btn-block btn-info']", u"onclick", u"all")
            for i in range(len(varPost)):
                ...
                if self.Level_PO.isElementId(u"approval-item-" + varObjectId):
                    self.Level_PO.clickXPATH(u"//div[@id='approval-item-" + varObjectId + u"']/i[1]", 2)
                    self.Level_PO.clickXPATH(u"//button[@class='btn btn-sm btn-primary']", 6)
            return u"已取消 -> 所有审批人"
            # self.varIsAudit = u"N"
        ...
        # 取消审批人
                ...
                for j in range(int(varIsComma)+1):
                    ...
                    for dictKey in dict1:
                        if varAudit == str(dict1[dictKey]):
                            ...
                            if self.Level_PO.isElementId(u"approval-item-" + varKey):
                                self.Level_PO.clickXPATH(u"//div[@id='approval-item-" + varKey + u"']/i[1]", 2)
            ...
            self.Level_PO.clickXPATH(u"//button[@class='btn btn-sm btn-primary']", 2)
        ...
        # 点击审批人
            ...
            if int(varIsComma) > 0:
                for j in range(int(varIsComma) + 1):
                    ...
                    try:
                        self.Level_PO.clickLINKTEXT(varTmp, 2)
                ...
                # 添加单个审批人
                self.Level_PO.clickLINKTEXT(varObjects[0], 2)
