...
# 环境变量
File_ExcelName = "C:\\Python27\\TMPappium\\excel\\jhj_1_6.xls"
Html_Testreport = "C:\\Python27\\TMPappium\\report\\testreport.html" # TestReport文件
Err_Screenshot = "C:\\Python27\\TMPappium\\screenshot\\"  # 错误截屏
#os.path.abspath() 方法用于获取当前路径下的文件
#PATH = lambda p: os.path.abspath(p)
PATH = lambda p: os.path.abspath(os.path.join(os.path.dirname(__file__), p))

class Jhj1_6(unittest.TestCase):
...
##############################################################################################
    ...
    def MySetupLogin(self):
        ...
        sleep(2)
        # 登录注册
        if self.isElement("cn.jihaojia:id/login_btn")==True :
            self.driver.find_element_by_id("cn.jihaojia:id/login_btn").click()
            self.Login("13816103333")
            sleep(4)
    def MySetupAccount(self):
         #C5-2,设置中检查账号资料、头像图、昵称、性别
         sleep(3)
         self.driver.find_element_by_id("cn.jihaojia:id/setting").click()
    ...
    def MySetupAboutus(self):
        #C5-3 关于我们
        self.driver.find_element_by_id("cn.jihaojia:id/we_jihaojia").click()
        b = Jhj15_mysetup(self.driver)
        b.aboutus()
    def MySetupFeedback(self):
        #C5-4 意见反馈
        # 检查标题
        sleep(3)
        if self.driver.find_element_by_id("cn.jihaojia:id/t1").text<>"意见反馈":
            print "Err,C5-4,意见反馈,标题不存在!"
        if self.isElement("cn.jihaojia:id/backbtncomm")==False:
            print "Err,C5-4,意见反馈,返回按钮不存在!"
        self.driver.find_element_by_id("cn.jihaojia:id/feedback_layout").click()
        # 类型选择 ？
        # 描述内容
        self.driver.find_element_by_id("cn.jihaojia:id/shopping_proble_edt").send_keys(u"1234567890")
        # 添加3张图
        self.driver.find_element_by_id("cn.jihaojia:id/add_after_img").click() # 点击 +
        self.driver.tap([(600, 300)], 50)
        self.driver.tap([(900, 300)], 50)
        self.driver.tap([(600, 500)], 50)
        self.driver.find_element_by_id("cn.jihaojia:id/id_ok").click()
        ...
        self.driver.find_elements_by_id("cn.jihaojia:id/id_item_close")[2].click()
        self.driver.find_elements_by_id("cn.jihaojia:id/id_item_close")[1].click()
        self.driver.find_element_by_id("cn.jihaojia:id/btn_pos").click() # 确定
        if self.isElement("cn.jihaojia:id/id_item_close")==False:
             print "Err,C5-4,添加的图片不存在!"
        # 输入手机号码
    ...
    def MySetupExitLogin(self):
        #C5-6 我的设置-注销账号
        self.driver.find_element_by_id("cn.jihaojia:id/gaga_login").click()
        if self.isElement("cn.jihaojia:id/login_btn")==False:
            print "Err,C5-6,注销后返回界面错误!"
    def Login(self,mobilephone):
        #C5-1 我的设置-获取验证码并自动登录
        self.driver.find_element_by_id("cn.jihaojia:id/backbtncomm").click()
        self.driver.find_element_by_id("cn.jihaojia:id/login_btn").click()
        self.driver.find_element_by_id("cn.jihaojia:id/gaga_login_account_edit").send_keys(mobilephone)
        self.driver.find_element_by_id("cn.jihaojia:id/codeButton").click() #获取验证码
        sleep(3)
        try:
            conn=MySQLdb.connect(host='192.168.0.100',user='root',passwd='',db='jtx001',port=3306)
            cur=conn.cursor()
