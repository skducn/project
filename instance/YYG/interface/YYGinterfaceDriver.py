...
# Author        : John
# Date          : 2016-7-29
# Description   : 医云谷1.0接口驱动
# *******************************************************************************************************************************

from YYGinterfaceParam import *
from xlutils.copy import copy

def response( interName, *rtnMessageStatus):
    m1 = md5.new()
    ...
    if u"操作成功" in str(resp):
        print interName + u" => [OK]"
        if rtnMessageStatus == ('param',):
            print u"[paramIN] => " + url + u" , " + str(values)
            print u"[paramOut] => " + str(resp)
    else:
        print interName + u" => [errorrrrrrrrrr]"
        print u"[paramIN] => " + url + u" , " + str(values)
        print u"[paramOut] => " + str(resp)
    return resp


def Icase(exlNo, RtnStatus, exlParams):
    ...
    # 获取接口总数量及接口序号明细
    for i in range(1,sheetNum.nrows):
    ...
    if floatExlNo in l_exlAllInterfaceNum:
        for j in range(0, sheetNum.nrows):
            ...
            if sheetNum.cell_value(j, 0) == floatExlNo :
                exlInterfaceName = sheetNum.cell_value(j, 2)
                exlInterfaceUrl = sheetNum.cell_value(j, 3)
        ...
        if varThirdway == 1 and exlIsCancel != 1:
            ...
            requestURL = varURL + exlInterfaceUrl
