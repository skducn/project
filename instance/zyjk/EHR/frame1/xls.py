...
class XLS:
    ...
    def getCaseParam(self):
        ...
        for i in range(1, self.sheet2.nrows):
            if self.sheet2.cell_value(i, 0) == 'N' or self.sheet2.cell_value(i, 0) == 'n':
                ...
                l_case.append(self.sheet2.cell_value(i, 13))  # checkDB
                l_case.append(self.sheet2.cell_value(i, 15))  # execDB
    ...
    def setCaseParam(self, excelNo, generation, result, response, checkDB, execDB):
        '''
        ...
        if checkDB == 0 :  # checkDB
            self.wSheet.write(excelNo, 14, checkDB, self.styleRed)
        else:
            self.wSheet.write(excelNo, 14, checkDB, self.styleBlue)
        if execDB =='done':  # execDB
            self.wSheet.write(excelNo, 16, execDB, self.styleBlue)
        else:
            self.wSheet.write(excelNo, 16, execDB, self.styleRed)
