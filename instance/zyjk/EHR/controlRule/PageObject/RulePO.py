...
class RulePO(object):
    ...
    def ruleId_17a7929801e54f1ca8ab69f18c086b00_2(self, comment, ruleSql, idCardNo):
        # 药物名称未填写
        ...
        try:
            ...
            self.runRule("正向", ruleSql, idCardNo, "检查点1（正向），本次随访管理状态代码 = Null，药物名称 = Null")

            # 检查点2（正向）
            self.execQuery("update tb_dc_dm_visit set vistStatusCode=1 where ehrNum='%s'" % ehrNum[0][0])
            self.execQuery("update tb_dc_dm_usedrug set drugName=Null")
            self.runRule("正向", ruleSql, idCardNo, "检查点2（正向），本次随访管理状态代码 = 1，药物名称 = Null")

            # 检查点3（反向）
            self.execQuery("update tb_dc_dm_visit set vistStatusCode=1 where ehrNum='%s'" % ehrNum[0][0])
            self.execQuery("update tb_dc_dm_usedrug set drugName = '111111'")
            self.runRule("", ruleSql, idCardNo, "检查点3（正向），本次随访管理状态代码 = 1，药物名称 != Null")

            # 检查点4（反向）
            self.execQuery("update tb_dc_dm_visit set vistStatusCode=2 where ehrNum='%s'" % ehrNum[0][0])
            self.execQuery("update tb_dc_dm_usedrug set drugName=Null")
            self.runRule("", ruleSql, idCardNo, "检查点4（正向），本次随访管理状态代码 = 2，药物名称 = Null")

            # 检查点5（反向）
            self.execQuery("update tb_dc_dm_visit set vistStatusCode=2 where ehrNum='%s'" % (ehrNum[0][0]))
            self.execQuery("update tb_dc_dm_usedrug set drugName = '111111'")
            self.runRule("", ruleSql, idCardNo, "检查点5（反向），本次随访管理状态代码 = 2，药物名称 != Null")
        except:
            Color_PO.consoleColor("31", "33", "[WARNING]", "数据库执行时报错!")
            exit()
    def ruleId_ac849eaa074545bd862680bf8b76242f_3(self, comment, ruleSql, idCardNo):
        # 药品名称与药物类型不匹配
        print("\n" + comment + ruleSql.center(100, "-"))
        try:
            # 1，数据准备
