...
class Qypt_web_PO():
    ...
    def clkMenu(self, varMenu, varMenu2):

        l_tmp = []
        l_tmp2 = []
        l_menu2 = []
        Web_PO.swhLabel(1)
        l_span = Web_PO.getTexts("//span")
        for i in range(len(l_span)):
            if l_span[i] != '':
                l_tmp.append(l_span[i])
        print(l_tmp)
        # print(list(enumerate(l_tmp, start=0)))
        d1 = (dict(enumerate(l_tmp, start=0)))
        d1 = {v: k for k, v in d1.items()}
        print(d1)

        # 点击一级菜单，获取二级菜单
        Web_PO.clk("/html/body/div/section/div/section/aside/section/main/div/div[1]/div/ul/li[" + str(d1[varMenu]) + "]/div", 4)
        # /html/body/div/section/div/section/aside/section/main/div/div[1]/div/ul/li[4]/div
        # l_span = Web_PO.getTexts("//span")
        # print(l_span)
        # l_menu2 = Web_PO.getTexts(u"//li[@role='menuitem']")
        # l_menu2 = Web_PO.getTexts(u"//ul[@class='el-menu el-menu--inline']")
        l_menu2 = Web_PO.getTexts(u"//li[@class='el-menu-item-group']")
        print(l_menu2)
        l_tmp2 = l_menu2[0].split("\n")
        print(l_tmp2)
        d2 = (dict(enumerate(l_tmp2, start=1)))
        d2 = {v: k for k, v in d2.items()}
        print(d2)

        # 点击二级菜单
        Web_PO.clk("/html/body/div/section/div/section/aside/section/main/div/div[1]/div/ul/li[" + str(d1[varMenu]) + "]/ul/li/ul/a[" + str(d2[varMenu2]) + "]", 2)

        # 关闭一级菜单
        Web_PO.clk("/html/body/div/section/div/section/aside/section/main/div/div[1]/div/ul/li[" + str(d1[varMenu]) + "]/div", 2)
