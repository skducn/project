...
# Author     : John
# Date       : 2022-10-12
# Description: # 定时任务的利器 apscheduler
...
# 触发器(trigger):定义了任务执行的时间表。可以基于日期、固定时间间隔、Cron表达式等来定义触发器。可以选择cron、date、interval中的一个。
# cron表示的是定时任务，类似linux crontab，在指定的时间触发。如：每天7点20分执行一次：task.add_job(func=sch_test, args=('定时任务',), trigger='cron',hour='7', minute='20')
# date表示具体到某个时间的一次性任务
# interval表示的是循环任务，指定一个间隔时间，每间隔时间执行一次。如：每隔3秒执行一次sch_test任务 　task.add_job(func=sch_test, args=('循环任务',), trigger='interval', seconds=3)。
# 任务存储器（JobStore）： 存储任务的调度状态。任务可以持久化到数据库或内存中。
...
# 调度器属于apscheduler的核心，它扮演着统筹整个apscheduler系统的角色，存储器、执行器、触发器在它的调度下正常运行。
