...
class JsonPO():
    ...
    def json2file(self, varUrl, varData, varFile):
        ...
        res = requests.get(url=varUrl, headers={'user-agent': Data_PO.getUserAgent()}, proxies={"url": Data_PO.getIpAgent()})
...
if __name__ == '__main__':
    ...
    print(Json_PO.getValueFromFileByExpr("data/dict.json", "stu_info[0].name"))  # ['小白']
    #
    ...
    x = (DeepDiff(v1, v2))
    print(x)  # {'values_changed': {"root['error_code']": {'new_value': 1, 'old_value': 0}, "root['stu_info'][0]['info']['card']": {'new_value': 434345432000, 'old_value': 434345432}}}
    print(x['values_changed'])  # {"root['error_code']": {'new_value': 1, 'old_value': 0}, "root['stu_info'][0]['info']['card']": {'new_value': 434345432000, 'old_value': 434345432}}

    print("3.2 比较2个json对象，对指定位置元素的差异检查".center(100, "-"))
    x = (DeepDiff(v1, v2, exclude_paths=["root['stu_info'][0]['info']"]))
    print(x)  # {'values_changed': {"root['error_code']": {'new_value': 1, 'old_value': 0}}}
    print(x['values_changed'])  # {"root['error_code']": {'new_value': 1, 'old_value': 0}}
    ...
    print(x)  # {'values_changed': {"root['_node']": {'new_value': 888, 'old_value': 123}}}
    print(x['values_changed'])  # {"root['_node']": {'new_value': 888, 'old_value': 123}}
    ...
    x = (DeepDiff(v1, v2))
    print(x)  # {'values_changed': {"root['level1']": {'new_value': 0.999, 'old_value': 0.998}}}


    print("3.4 比较2个json对象，忽略对指定类型数据的比较检查".center(100, "-"))
    from datetime import datetime,timedelta
    v1 = {"x": datetime.strptime('2022-08-07','%Y-%m-%d'),
          'y': timedelta(seconds=100)}

    v2 = {"x": datetime.strptime('2032-03-17','%Y-%m-%d'),
