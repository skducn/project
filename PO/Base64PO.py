...
class Base64PO:
    ...
    def decodeImg(self, src, filename='test'):
        """
        解码图片
        :param src: 图片编码
            eg:
        ...
        # 3、二进制文件保存 os.getcwd()
        filename = "{}.{}".format(os.getcwd() + "/" + filename, ext)
        with open(filename, "wb") as f:
            f.write(img)

        return filename

    def encodeImg(self, filename):
        """
        编码图片
...
if __name__ == "__main__":
    ...
    Base64_PO.decodeImg(url, "123")
    # decode_image(url)

    # # 编码（转换成base64）
    # print(Base64_PO.encodeImg("123.gif"))
